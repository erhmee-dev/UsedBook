...
Class Zone extends DBConnection {
	...
	public function save_zone(){
		...
		if(empty($id)){
			...
			if($chk > 0){
				...
				if($qry){
					$this->settings->set_flashdata('success','zone амжилттай хадгалагдсан.');
					return 1;
				}else{
			...
			if($chk > 0){
				...
				if($qry){
					$this->settings->set_flashdata('success','zone амжилттай шинэчлэгдтэлээ.');
					return 1;
				}else{
					return "INSERT INTO barangay_list set {$data}";
				}
	...
	public function delete_zone(){
		...
		if($qry){
			$this->settings->set_flashdata('success','zone амжилттай устагллаа .');
			return 1;
		}else{
			return false;
		}
