...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				$this->settings->set_flashdata('success','Хэрэглэгчийн мэдээллийг амжилттай хадгаллаа.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
			...
			if($qry){
				$this->settings->set_flashdata('success','Хэрэглэгчийн мэдээллийг амжилттай шинэчлэгдтэлээ .');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
						$this->settings->set_userdata($k,$v);
	...
	public function delete_users(){
		...
		if($qry){
			$this->settings->set_flashdata('success','Хэрэглэгчийн мэдээллийг амжилттай устгалаа.');
			return 1;
		}else{
			return false;
		}
